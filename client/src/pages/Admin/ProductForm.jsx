...
import { debugRequest, debugResponse, validateProductData } from '../../utils/debugUtils';

// Default categories as fallback
const DEFAULT_CATEGORIES = [
  { _id: 'power-tools', name: 'Power Tools' },
  { _id: 'hand-tools', name: 'Hand Tools' },
  { _id: 'measuring-tools', name: 'Measuring Tools' },
  { _id: 'safety-equipment', name: 'Safety Equipment' },
  { _id: 'plumbing', name: 'Plumbing' },
  { _id: 'electrical', name: 'Electrical' },
  { _id: 'paint-finishes', name: 'Paint & Finishes' },
  { _id: 'building-materials', name: 'Building Materials' },
  { _id: 'fasteners', name: 'Fasteners' },
  { _id: 'garden-tools', name: 'Garden Tools' },
];
...
const ProductForm = () => {
  ...
  const isEditMode = !!id;
  console.log('ProductForm loaded, isEditMode:', isEditMode, 'id:', id);
  ...
  // Fetch categories - use simpler approach
  const categoriesQuery = useQuery({
    queryKey: ['all-categories-for-dropdown'],
    queryFn: async () => {
      try {
  ...
  console.log('🎯 Is loading:', categoriesQuery.isLoading);
  ...
  const saveMutation = useMutation({
    mutationFn: async (data) => {
      console.log('🔐 mutationFn called, isEditMode:', isEditMode);
      if (isEditMode) {
        console.log('📤 PUT to:', `/products/${id}`);
    ...
    onError: (error) => {
      console.error('❌ onError:', error.response?.data?.message || error);
  ...
  const handleDirectSave = async () => {
    ...
    } else if (uploadMode === 'file' && uploadedFiles.length > 0) {
      // Use uploaded files that have successful uploads
    ...
    // Validate that at least one image is provided if in URL mode and URLs were entered
    if (uploadMode === 'url' && imageUrls.some(url => url.trim()) && images.length === 0) {
    ...
    const productData = {
      ...
      category: formData.category, // This should be category ID
    ...
    const validationIssues = validateProductData(productData);
    ...
    debugRequest('/products', 'post', productData, axiosClient.defaults.headers);
  ...
  const handleSubmit = (e) => {
    ...
    // For now, call handleDirectSave directly
    handleDirectSave();
  };
