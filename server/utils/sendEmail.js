...
// Create reusable transporter - use Gmail service directly
const createTransporter = async () => {
  ...
  if (!emailPass) {
    console.error('❌ Email password not set!');
  }
...
const getTransporter = async () => {
  if (!transporterInstance) {
    ...
    try {
      ...
      logger.info(`✅ Gmail connected: ${process.env.EMAIL_USER}`);
    } catch (err) {
      logger.error(`❌ Gmail connection failed: ${err.message}`);
    }
  }
...
export const sendEmail = async ({ to, subject, text, html }) => {
  try {
    ...
    logger.error(`Full email error:`, error.response?.responseCode, error.response?.response);
    throw error;
  }
};
