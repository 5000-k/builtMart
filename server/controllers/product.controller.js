...
const createProduct = asyncHandler(async (req, res) => {
  ...
  if (productData.images && Array.isArray(productData.images)) {
    console.log('🖼️ Processing images:', productData.images.length);
    
    // Filter out invalid images
    const validImages = productData.images.filter(img => {
      if (!img.public_id || !img.url) {
        console.warn('⚠️ Invalid image found:', img);
        return false;
      }
      return true;
    });
    
    if (validImages.length !== productData.images.length) {
      console.warn('⚠️ Some images were filtered out due to missing public_id or url');
